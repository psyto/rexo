# Context Capital — ERC-8004 re-execution validator (PoC)

Proves that Context Capital plugs into the **ERC-8004 Validation Registry** as an
...
2. `ValidationRegistry.validationRequest(validator, agentId, requestURI, requestHash)` — the operator asks Context Capital to validate a job.
3. `ValidationRegistry.validationResponse(requestHash, 100, responseURI, responseHash, "reexec")` — **Context Capital, the named validator, posts the re-computed result on-chain.**
...
that Context Capital produced from `fixtures/reckn-r1-bundle`. On-chain carries
